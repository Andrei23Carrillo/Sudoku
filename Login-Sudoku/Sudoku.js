...
function iniciar_juego_primera_vez_2()
{
    ...
    alert('Bienvenido al Yasminoku.\nPara crear sudokus aleatorios: introduce la cantidad de numeros iniciales y pulsa \'Crear\'.\nAtencion: este juego podria llegar a generar sudokus con mas de una solucion (puedes comprobarlo dandole diversas veces en \'intentar solucionar\', y si siempre genera la misma solucion significara que lo mas probable es que el sudoku solo tenga una solucion).\nPara resolver un sudoku propio: introduce los numeros iniciales a 0, crea el sudoku que quieras solucionar y luego dale a \'intentar solucionar\'.');
...
function reiniciar_juego()
{
    ...
    if (confirm("Pulsa aceptar para aplicar los cambios. Se perdera la partida actual."))
    {
        //Vuelve a mostrar la opcion predeterminada en el div de ver solucion:
        document.getElementById('btnsolucion').innerHTML = "ver soluci&oacute;n";
        document.getElementById('btnsolucion').title = 'Muestra el sudoku resuelto';
        ver_solucion = false;
...
function validar_sudoku_usuario()
{
    ...
    if (!sudoku_completado) { alert("Debes acabar el sudoku para poder validarlo."); }
    //...pero si se han completado todos, se valida:
    else
    {
        var sudoku_valido = validar_sudoku(true);
        if (sudoku_valido) { alert("El sudoku ES CORRECTO. Enhorabuena."); } //Si es correcto, da la enhorabuena.
        else { alert("El sudoku NO ES CORRECTO."); } //Si no, avisa.
    }
}
...
function mostrar_solucion()
{
    ...
    if (ver_solucion)
    {
        ...
        if (!confirm("Se va a mostrar la solucion que el ordenador ha calculado para este sudoku (sin tener en cuenta los numeros introducidos por el usuario).\nSi prefieres intentar calcular en tiempo real una solucion, teniendo en cuenta los numeros introducidos por el usuario, pulsa en \"intentar solucionar\": esta opcion podria dar una solucion distinta al pulsarse mas de una vez si el sudoku tiene varias.\nPulsa aceptar para ver la solucion."))
        {
        ...
        document.getElementById("btnsolucion").innerHTML = "ocultar soluci&oacute;n";
        document.getElementById('btnsolucion').title = 'Oculta el sudoku resuelto';
        for (j=1; j<=81; j++)
        {
    ...
    document.getElementById('btnsolucion').style.color = '#222222';
}
...
function intentar_solucionar()
{
    ...
    if (!confirm("Se va a intentar solucionar el sudoku con los numeros introducidos, iniciales o no (puestos por el usuario).\nPodria llegar a dar una solucion distinta cada vez que se pulse esta opcion si el sudoku tiene varias soluciones.\nEs posible que el navegador deje de responder por un tiempo, no lo canceles.\nSi el navegador pregunta si abortar la secuencia de comandos del script, dile que no.\nPulsa aceptar para continuar.")) { return; }

    //Se muestra el mensaje de solucionando:
    mostrar_mensaje("Solucionando...", true);
    ...
    if (sudoku_solucionado && sudoku_valido)
    {
        ...
        alert("Sudoku solucionado.\nPulsa aceptar para dejar de ver la solucion.");
    ...
    else { mostrar_mensaje("", false); alert("Imposible solucionar.\nEl sudoku introducido no tiene solucion o es incorrecto."); }

    //Vuelve el color del "enlace" a su color predeterminado (si no se quedaria en rojo si se arrastrara con el raton, etc. en algunos navegadores y determinadas circunstancias):
    document.getElementById('btnsolucionar').style.color = '#222222';
}
